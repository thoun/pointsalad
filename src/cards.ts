...
function typeWithLeast(param: number[]): string {
    return formatTextIcons(`<div class="flex">[${param[1]}]<span>/</span><span>${_('Veggie type with at least ${least}').replace('${least}', param[0])}</span></div>`);
}

function highestTotal(): string {
    return formatTextIcons(`<div class="flex"><span>${_('Most total veggie')}</span> = [10]</div>`);
}

function lowestTotal(): string {
    return formatTextIcons(`<div class="flex"><span>${_('Lowest veggie total')}</span> = [7]</div>`);
}

function completeSet(): string {
    return formatTextIcons(`
